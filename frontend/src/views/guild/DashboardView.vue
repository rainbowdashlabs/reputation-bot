...
<template>
  <div class="mx-auto space-y-8 py-8" style="max-width: 1200px;">
    <div class="flex items-center justify-between">
      <Header1 class="text-3xl font-bold text-gray-900 dark:text-white">{{ t('dashboard.title') }}</Header1>
    </div>
    ...
    <div v-else-if="stats" class="space-y-8">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatsCard :title="t('dashboard.totalReputation')" :value="stats.totalReputation" border-color="border-blue-500" />
        <StatsCard :title="t('dashboard.weekReputation')" :value="stats.weekReputation" border-color="border-indigo-500" />
        <StatsCard :title="t('dashboard.todayReputation')" :value="stats.todayReputation" border-color="border-purple-500" />
