...
public class Add extends BaseReputationModifier {
    ...
    public Add(RoleAssigner roleAssigner, GuildRepository guildRepository) {
        super(roleAssigner, guildRepository);
    }

    @Override
    void execute(SlashCommandInteractionEvent event, EventContext context, User user, RepMember repMember, long rep) {
        repMember.addReputation(rep);
        event.reply(context.localize(
                        "command.repadmin.reputation.add.message.added",
