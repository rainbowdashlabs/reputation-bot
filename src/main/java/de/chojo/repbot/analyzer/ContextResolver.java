...
public class ContextResolver {
    ...
    private void addLatestAuthors(MessageContext context, @Nullable Settings settings) {
        ...
        var limit = settings == null
                ? configuration.analyzerSettings().historySize()
                : settings.abuseProtection().minMessages();
    ...
    private void addRecentAuthors(MessageContext context, Settings settings) {
        var maxAge = Instant.now()
                .minus(
                        settings == null
                                ? Long.MAX_VALUE
                                : settings.abuseProtection().maxMessageAge(),
                        ChronoUnit.MINUTES);
    ...
    private void addMembersAfter(Collection<Message> messages, MessageContext context, Instant oldest) {
        ...
        var memberIds = filtered.stream()
                ...
                .distinct()
                .map(u -> context.guild()
                        .retrieveMemberById(u.getIdLong())
                        .onErrorMap(mes -> null)
                        .complete())
    ...
    private MessageContext retrieveVoiceContext(Member target, Message message, @Nullable Settings settings) {
        ...
        if (voiceState.inAudioChannel()) {
            var voice = voiceState.getChannel().getMembers().stream()
                    .map(Member::getIdLong)
                    .collect(Collectors.toSet());
            context.addIds(voice);
        }
        var pastUser = voiceData.getPastUser(
                target.getUser(),
                message.getGuild(),
                settings == null ? 0 : settings.abuseProtection().minMessages(),
                configuration.analyzerSettings().voiceMembers());
        context.addIds(pastUser.stream()
                .map(id -> message.getGuild()
