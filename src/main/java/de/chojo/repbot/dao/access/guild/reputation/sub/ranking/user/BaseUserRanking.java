...
public abstract class BaseUserRanking implements GuildHolder {
    private final UserRanking user;

    protected BaseUserRanking(UserRanking user) {
        this.user = user;
    }
    ...
    public GuildRanking defaultRanking(int pageSize, Member member) {
        return byMode(user.ranking().reputation().repGuild().settings().general().reputationMode(), pageSize, member);
    }

    public GuildRanking byMode(ReputationMode mode, int pageSize, Member member) {
        return switch (mode) {
     ...
     */
    public GuildRanking total(int pageSize, Member member) {
     ...
     * @return a sorted list of reputation users
     */
    public GuildRanking days7(int pageSize, Member member) {
     ...
     * @return a sorted list of reputation users
     */
    public GuildRanking days30(int pageSize, Member member) {
     ...
     * @return a sorted list of reputation users
     */
    public GuildRanking week(int pageSize, Member member) {
     ...
     * @return a sorted list of reputation users
     */
    public GuildRanking month(int pageSize, Member member) {
        return getGuildRanking("command.top.message.monthTitle", pageSize, member, ReputationMode.MONTH);
    }

    private GuildRanking getGuildRanking(String title, int pageSize, Member member, ReputationMode mode){
