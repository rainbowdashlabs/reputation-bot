...
public record SystemInfoStatisticDisplay(ShardCountStatistic shardCountStatistic, DataStatistic dataStatistic)
    ...
    public void appendTo(EmbedBuilder embedBuilder) {
        embedBuilder
                ...
                .appendDescription(String.format(
                        "Watching %s guilds on %s shard/s\n%s/%s active channel on %s active guilds.",
                        dataStatistic.guilds(),
                        shardCountStatistic.shardCount(),
                        dataStatistic.activeChannel(),
                        dataStatistic.channel(),
                        dataStatistic.activeGuilds()));
    }
}
